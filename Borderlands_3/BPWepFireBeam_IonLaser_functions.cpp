#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BPWepFireBeam_IonLaser

#include "Basic.hpp"

#include "BPWepFireBeam_IonLaser_classes.hpp"
#include "BPWepFireBeam_IonLaser_parameters.hpp"


namespace SDK
{

// Function BPWepFireBeam_IonLaser.BPWepFireBeam_IonLaser_C.ExecuteUbergraph_BPWepFireBeam_IonLaser
// (HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBPWepFireBeam_IonLaser_C::ExecuteUbergraph_BPWepFireBeam_IonLaser(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BPWepFireBeam_IonLaser_C", "ExecuteUbergraph_BPWepFireBeam_IonLaser");

	Params::BPWepFireBeam_IonLaser_C_ExecuteUbergraph_BPWepFireBeam_IonLaser Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BPWepFireBeam_IonLaser.BPWepFireBeam_IonLaser_C.EvaluateShot
// (BlueprintCallable, BlueprintEvent)

void UBPWepFireBeam_IonLaser_C::EvaluateShot()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BPWepFireBeam_IonLaser_C", "EvaluateShot");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BPWepFireBeam_IonLaser.BPWepFireBeam_IonLaser_C.K2_OnDeactivated
// (Event, Public, BlueprintEvent)

void UBPWepFireBeam_IonLaser_C::K2_OnDeactivated()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BPWepFireBeam_IonLaser_C", "K2_OnDeactivated");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BPWepFireBeam_IonLaser.BPWepFireBeam_IonLaser_C.NotifyUseFinished_Event
// (BlueprintCallable, BlueprintEvent)

void UBPWepFireBeam_IonLaser_C::NotifyUseFinished_Event()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BPWepFireBeam_IonLaser_C", "NotifyUseFinished_Event");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BPWepFireBeam_IonLaser.BPWepFireBeam_IonLaser_C.NotifyUseStarted_Event
// (BlueprintCallable, BlueprintEvent)

void UBPWepFireBeam_IonLaser_C::NotifyUseStarted_Event()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BPWepFireBeam_IonLaser_C", "NotifyUseStarted_Event");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BPWepFireBeam_IonLaser.BPWepFireBeam_IonLaser_C.ReceiveBeginPlay
// (Event, Public, BlueprintEvent)

void UBPWepFireBeam_IonLaser_C::ReceiveBeginPlay()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BPWepFireBeam_IonLaser_C", "ReceiveBeginPlay");

	UObject::ProcessEvent(Func, nullptr);
}

}

