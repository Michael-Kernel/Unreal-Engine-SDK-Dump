#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: LightProjectile_AmberManagement_Healing

#include "Basic.hpp"

#include "LightProjectile_AmberManagement_Healing_classes.hpp"
#include "LightProjectile_AmberManagement_Healing_parameters.hpp"


namespace SDK
{

// Function LightProjectile_AmberManagement_Healing.LightProjectile_AmberManagement_Healing_C.EnableProjectileHoming
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// class UOakLightProjectile*              LightProjectile                                        (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void ULightProjectile_AmberManagement_Healing_C::EnableProjectileHoming(class UOakLightProjectile* LightProjectile)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("LightProjectile_AmberManagement_Healing_C", "EnableProjectileHoming");

	Params::LightProjectile_AmberManagement_Healing_C_EnableProjectileHoming Parms{};

	Parms.LightProjectile = LightProjectile;

	UObject::ProcessEvent(Func, &Parms);
}


// Function LightProjectile_AmberManagement_Healing.LightProjectile_AmberManagement_Healing_C.TryEnableHoming
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// class UOakLightProjectile*              LightProjectile                                        (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void ULightProjectile_AmberManagement_Healing_C::TryEnableHoming(class UOakLightProjectile* LightProjectile)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("LightProjectile_AmberManagement_Healing_C", "TryEnableHoming");

	Params::LightProjectile_AmberManagement_Healing_C_TryEnableHoming Parms{};

	Parms.LightProjectile = LightProjectile;

	UObject::ProcessEvent(Func, &Parms);
}


// Function LightProjectile_AmberManagement_Healing.LightProjectile_AmberManagement_Healing_C.OnLifetimeExpired
// (Event, Public, HasDefaults, BlueprintCallable, BlueprintEvent, Const)
// Parameters:
// class ULightProjectile*                 Projectile                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void ULightProjectile_AmberManagement_Healing_C::OnLifetimeExpired(class ULightProjectile* Projectile) const
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("LightProjectile_AmberManagement_Healing_C", "OnLifetimeExpired");

	Params::LightProjectile_AmberManagement_Healing_C_OnLifetimeExpired Parms{};

	Parms.Projectile = Projectile;

	UObject::ProcessEvent(Func, &Parms);
}

}

