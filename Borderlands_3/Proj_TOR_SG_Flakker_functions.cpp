#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: Proj_TOR_SG_Flakker

#include "Basic.hpp"

#include "Proj_TOR_SG_Flakker_classes.hpp"
#include "Proj_TOR_SG_Flakker_parameters.hpp"


namespace SDK
{

// Function Proj_TOR_SG_Flakker.Proj_TOR_SG_Flakker_C.ExecuteUbergraph_Proj_TOR_SG_Flakker
// (HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void AProj_TOR_SG_Flakker_C::ExecuteUbergraph_Proj_TOR_SG_Flakker(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Proj_TOR_SG_Flakker_C", "ExecuteUbergraph_Proj_TOR_SG_Flakker");

	Params::Proj_TOR_SG_Flakker_C_ExecuteUbergraph_Proj_TOR_SG_Flakker Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function Proj_TOR_SG_Flakker.Proj_TOR_SG_Flakker_C.Stage_03
// (BlueprintCallable, BlueprintEvent)

void AProj_TOR_SG_Flakker_C::Stage_03()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Proj_TOR_SG_Flakker_C", "Stage_03");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Proj_TOR_SG_Flakker.Proj_TOR_SG_Flakker_C.Stage_02
// (BlueprintCallable, BlueprintEvent)

void AProj_TOR_SG_Flakker_C::Stage_02()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Proj_TOR_SG_Flakker_C", "Stage_02");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Proj_TOR_SG_Flakker.Proj_TOR_SG_Flakker_C.Stage_01
// (BlueprintCallable, BlueprintEvent)

void AProj_TOR_SG_Flakker_C::Stage_01()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Proj_TOR_SG_Flakker_C", "Stage_01");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Proj_TOR_SG_Flakker.Proj_TOR_SG_Flakker_C.OnExplode
// (Event, Protected, BlueprintEvent)

void AProj_TOR_SG_Flakker_C::OnExplode()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Proj_TOR_SG_Flakker_C", "OnExplode");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Proj_TOR_SG_Flakker.Proj_TOR_SG_Flakker_C.ReceiveHit
// (Event, Public, HasOutParams, BlueprintEvent)
// Parameters:
// class UPrimitiveComponent*              MyComp                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class AActor*                           Other                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class UPrimitiveComponent*              OtherComp                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// bool                                    bSelfMoved                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
// struct FVector                          HitLocation                                            (BlueprintVisible, BlueprintReadOnly, Parm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// struct FVector                          HitNormal                                              (BlueprintVisible, BlueprintReadOnly, Parm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// struct FVector                          NormalImpulse                                          (BlueprintVisible, BlueprintReadOnly, Parm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// struct FHitResult                       Hit                                                    (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm, IsPlainOldData, NoDestructor, ContainsInstancedReference)

void AProj_TOR_SG_Flakker_C::ReceiveHit(class UPrimitiveComponent* MyComp, class AActor* Other, class UPrimitiveComponent* OtherComp, bool bSelfMoved, const struct FVector& HitLocation, const struct FVector& HitNormal, const struct FVector& NormalImpulse, const struct FHitResult& Hit)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Proj_TOR_SG_Flakker_C", "ReceiveHit");

	Params::Proj_TOR_SG_Flakker_C_ReceiveHit Parms{};

	Parms.MyComp = MyComp;
	Parms.Other = Other;
	Parms.OtherComp = OtherComp;
	Parms.bSelfMoved = bSelfMoved;
	Parms.HitLocation = std::move(HitLocation);
	Parms.HitNormal = std::move(HitNormal);
	Parms.NormalImpulse = std::move(NormalImpulse);
	Parms.Hit = std::move(Hit);

	UObject::ProcessEvent(Func, &Parms);
}


// Function Proj_TOR_SG_Flakker.Proj_TOR_SG_Flakker_C.ReceiveBeginPlay
// (Event, Protected, BlueprintEvent)

void AProj_TOR_SG_Flakker_C::ReceiveBeginPlay()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Proj_TOR_SG_Flakker_C", "ReceiveBeginPlay");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Proj_TOR_SG_Flakker.Proj_TOR_SG_Flakker_C.UserConstructionScript
// (Event, Public, BlueprintCallable, BlueprintEvent)

void AProj_TOR_SG_Flakker_C::UserConstructionScript()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Proj_TOR_SG_Flakker_C", "UserConstructionScript");

	UObject::ProcessEvent(Func, nullptr);
}

}

