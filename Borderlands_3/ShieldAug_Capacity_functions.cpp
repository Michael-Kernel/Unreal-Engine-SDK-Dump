#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: ShieldAug_Capacity

#include "Basic.hpp"

#include "ShieldAug_Capacity_classes.hpp"
#include "ShieldAug_Capacity_parameters.hpp"


namespace SDK
{

// Function ShieldAug_Capacity.ShieldAug_Capacity_C.GetValueForAttribute
// (Event, Protected, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// class UGbxAttributeData*                Attribute                                              (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class UObject*                          Context                                                (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// float                                   ReturnValue                                            (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

float UShieldAug_Capacity_C::GetValueForAttribute(const class UGbxAttributeData* Attribute, const class UObject* Context)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("ShieldAug_Capacity_C", "GetValueForAttribute");

	Params::ShieldAug_Capacity_C_GetValueForAttribute Parms{};

	Parms.Attribute = Attribute;
	Parms.Context = Context;

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}

}

