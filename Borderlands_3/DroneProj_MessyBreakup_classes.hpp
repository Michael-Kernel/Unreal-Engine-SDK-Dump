#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: DroneProj_MessyBreakup

#include "Basic.hpp"

#include "OakGame_classes.hpp"
#include "Engine_structs.hpp"
#include "CoreUObject_structs.hpp"


namespace SDK
{

// BlueprintGeneratedClass DroneProj_MessyBreakup.DroneProj_MessyBreakup_C
// 0x0070 (0x07A0 - 0x0730)
class ADroneProj_MessyBreakup_C final : public AOakDroneProjectile
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame;                                    // 0x0730(0x0008)(Transient, DuplicateTransient)
	class UOakDialogComponent*                    OakDialog;                                         // 0x0738(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	class UParticleSystemComponent*               Booster_FX;                                        // 0x0740(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	class UWwiseAudioComponent*                   WWisAudio;                                         // 0x0748(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	class USocketComponent*                       Muzzle;                                            // 0x0750(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	class UStaticMeshComponent*                   MyBody;                                            // 0x0758(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	class UWwiseEvent*                            CachedWeaponFireWwiseEvent;                        // 0x0760(0x0008)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, ExposeOnSpawn, HasGetValueTypeHash)
	class UClass*                                 DamageType;                                        // 0x0768(0x0008)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, ExposeOnSpawn, HasGetValueTypeHash)
	class UClass*                                 DamageSource;                                      // 0x0770(0x0008)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, ExposeOnSpawn, HasGetValueTypeHash)
	struct FVector                                TargetLocation;                                    // 0x0778(0x000C)(Edit, BlueprintVisible, DisableEditOnInstance, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         BulletDamage;                                      // 0x0784(0x0004)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, ExposeOnSpawn, HasGetValueTypeHash)
	float                                         FireRate;                                          // 0x0788(0x0004)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, ExposeOnSpawn, HasGetValueTypeHash)
	uint8                                         Pad_78C[0x4];                                      // 0x078C(0x0004)(Fixing Size After Last Property [ Dumper-7 ])
	class ABPWeap_Tediore_BaseWeapon_C*           Weapon;                                            // 0x0790(0x0008)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnTemplate, DisableEditOnInstance, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	class AShield*                                Shield;                                            // 0x0798(0x0008)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnTemplate, DisableEditOnInstance, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

public:
	void ExecuteUbergraph_DroneProj_MessyBreakup(int32 EntryPoint);
	void KilledEnemy(const class UDamageComponent* Damaged, const struct FCausedDeathDetails& Details);
	void OnExplode();
	void ShieldUnequippedEvent(class AOakCharacter* EquipInstigator);
	void OnBeginExplode();
	void BndEvt__DroneMovement_K2Node_ComponentBoundEvent_0_OakDroneProjectileFacingTargetChangedDelegate__DelegateSignature_DroneProj_TED_TenGallon(class AOakDroneProjectile* Drone);
	void ReceiveBeginPlay();
	void UserConstructionScript();
	void FireWeapon();
	void StartFire();
	void StopFire();
	void NoFriends();
	void DistanceCHeck();
	void IdleVO();
	void KillVO();

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"DroneProj_MessyBreakup_C">();
	}
	static class ADroneProj_MessyBreakup_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<ADroneProj_MessyBreakup_C>();
	}
};
static_assert(alignof(ADroneProj_MessyBreakup_C) == 0x000008, "Wrong alignment on ADroneProj_MessyBreakup_C");
static_assert(sizeof(ADroneProj_MessyBreakup_C) == 0x0007A0, "Wrong size on ADroneProj_MessyBreakup_C");
static_assert(offsetof(ADroneProj_MessyBreakup_C, UberGraphFrame) == 0x000730, "Member 'ADroneProj_MessyBreakup_C::UberGraphFrame' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, OakDialog) == 0x000738, "Member 'ADroneProj_MessyBreakup_C::OakDialog' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, Booster_FX) == 0x000740, "Member 'ADroneProj_MessyBreakup_C::Booster_FX' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, WWisAudio) == 0x000748, "Member 'ADroneProj_MessyBreakup_C::WWisAudio' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, Muzzle) == 0x000750, "Member 'ADroneProj_MessyBreakup_C::Muzzle' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, MyBody) == 0x000758, "Member 'ADroneProj_MessyBreakup_C::MyBody' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, CachedWeaponFireWwiseEvent) == 0x000760, "Member 'ADroneProj_MessyBreakup_C::CachedWeaponFireWwiseEvent' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, DamageType) == 0x000768, "Member 'ADroneProj_MessyBreakup_C::DamageType' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, DamageSource) == 0x000770, "Member 'ADroneProj_MessyBreakup_C::DamageSource' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, TargetLocation) == 0x000778, "Member 'ADroneProj_MessyBreakup_C::TargetLocation' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, BulletDamage) == 0x000784, "Member 'ADroneProj_MessyBreakup_C::BulletDamage' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, FireRate) == 0x000788, "Member 'ADroneProj_MessyBreakup_C::FireRate' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, Weapon) == 0x000790, "Member 'ADroneProj_MessyBreakup_C::Weapon' has a wrong offset!");
static_assert(offsetof(ADroneProj_MessyBreakup_C, Shield) == 0x000798, "Member 'ADroneProj_MessyBreakup_C::Shield' has a wrong offset!");

}

