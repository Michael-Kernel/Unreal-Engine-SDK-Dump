#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: Ability_Artifact_SparkPlug

#include "Basic.hpp"

#include "Ability_Artifact_SparkPlug_classes.hpp"
#include "Ability_Artifact_SparkPlug_parameters.hpp"


namespace SDK
{

// Function Ability_Artifact_SparkPlug.Ability_Artifact_SparkPlug_C.ExecuteUbergraph_Ability_Artifact_SparkPlug
// (HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UAbility_Artifact_SparkPlug_C::ExecuteUbergraph_Ability_Artifact_SparkPlug(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Ability_Artifact_SparkPlug_C", "ExecuteUbergraph_Ability_Artifact_SparkPlug");

	Params::Ability_Artifact_SparkPlug_C_ExecuteUbergraph_Ability_Artifact_SparkPlug Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function Ability_Artifact_SparkPlug.Ability_Artifact_SparkPlug_C.OnActivated
// (Event, Protected, BlueprintEvent)

void UAbility_Artifact_SparkPlug_C::OnActivated()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Ability_Artifact_SparkPlug_C", "OnActivated");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Ability_Artifact_SparkPlug.Ability_Artifact_SparkPlug_C.OnDeactivated
// (Event, Protected, BlueprintEvent)

void UAbility_Artifact_SparkPlug_C::OnDeactivated()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Ability_Artifact_SparkPlug_C", "OnDeactivated");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Ability_Artifact_SparkPlug.Ability_Artifact_SparkPlug_C.SlamEnd
// (HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// struct FGroundSlamEndedDetails          Details                                                (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm, NoDestructor)

void UAbility_Artifact_SparkPlug_C::SlamEnd(const struct FGroundSlamEndedDetails& Details)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Ability_Artifact_SparkPlug_C", "SlamEnd");

	Params::Ability_Artifact_SparkPlug_C_SlamEnd Parms{};

	Parms.Details = std::move(Details);

	UObject::ProcessEvent(Func, &Parms);
}

}

