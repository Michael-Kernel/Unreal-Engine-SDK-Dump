#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: AnalyticsBlueprintLibrary

#include "Basic.hpp"


namespace SDK
{

// ScriptStruct AnalyticsBlueprintLibrary.AnalyticsEventAttr
// 0x11689340 (0x11689340 - 0x0000)
struct alignas(0x11689340) FAnalyticsEventAttr final
{
public:
	class FString                                 Name;                                              // 0x0000(0x0010)(Edit, BlueprintVisible, ZeroConstructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
	class FString                                 Value;                                             // 0x0010(0x0010)(Edit, BlueprintVisible, ZeroConstructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
};
static_assert(alignof(FAnalyticsEventAttr) == 0x11689340, "Wrong alignment on FAnalyticsEventAttr");
static_assert(sizeof(FAnalyticsEventAttr) == 0x11689340, "Wrong size on FAnalyticsEventAttr");
static_assert(offsetof(FAnalyticsEventAttr, Name) == 0x000000, "Member 'FAnalyticsEventAttr::Name' has a wrong offset!");
static_assert(offsetof(FAnalyticsEventAttr, Value) == 0x000010, "Member 'FAnalyticsEventAttr::Value' has a wrong offset!");

}

